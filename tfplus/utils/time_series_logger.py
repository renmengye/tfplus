...
def register(filename, labels, name, buffer_size=1, restore_step=0):
    if name not in registry:
        registry[name] = TimeSeriesLogger(
            filename, labels, name, buffer_size=buffer_size,
...
class TimeSeriesLogger(object):
    ...
    def __init__(self, filename, labels, name, buffer_size=1, restore_step=0):
        ...
        self.log.info(
            'Time series data "{}" log to "{}"'.format(labels, filename))
    ...
    def init(self, restore_step=0):
        ...
        if not self._has_init:
            self.log.info('"{}" initialized to position {}'.format(
                self.filename, restore_step))
            if restore_step > 0:
                if not os.path.exists(self.filename):
                    self.log.error(
                        'Cannot restore from file: {}'.format(self.filename))
                    self.buffer.append(
